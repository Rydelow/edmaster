		...
		<div class="subject-inner wpsp-left wpsp-class-filter">
			<form name="ClassForm" id="ClassForm" method="post" action="">
				...
				<select name="ClassID" id="ClassID" class="wpsp-form-control">
					...
					<option value="all" <?php if($sel_classid=='all') echo esc_html("selected","wpschoolpress"); ?>><?php echo esc_html("All","wpschoolpress");?></option>

					<?php
					foreach( $sel_class as $classes ) { ?>

						<option value="<?php echo esc_attr($classes->cid);?>" <?php if($sel_classid==$classes->cid) echo esc_html("selected","wpschoolpress"); ?>>
							<?php echo esc_html($classes->c_name);?>
						</option>
					<?php }
					//if($current_user_role=='administrator' ) { ?>
					<!-- 	<option value="all" <?php if($sel_classid=='all') echo esc_html("selected","wpschoolpress"); ?>>All</option> -->
					<?php //} ?>
				</select>
			</form>
		</div>
		<div class="wpsp-right wpsp-import-export">
					<div class="wpsp-btn-lists"  title="<?php echo esc_attr($protitle);?>" <?php echo esc_html($prodisable);?>>
						...
						<div class="wpsp-button-group wpsp-dropdownmain wpsp-left">
							...
							<div class="wpsp-dropdown wpsp-dropdown-md">
								...
								<form id="ParentColumnForm" name="ParentColumnForm">
									<li class="wpsp-drop-title wpsp-checkList"> <?php echo esc_html("Select Columns to Print","wpschoolpress");?></li>
						...
						<div class="psp-dropdownmain wpsp-button-group">
							<button type="button" class="wpsp-btn print" id="ExportParents" <?php echo esc_html($prodisable);?> title="<?php echo esc_attr($protitle);?>"><i class="fa fa-download"></i> <?php echo esc_html("Export","wpschoolpress");?> </button>
							...
							<div class="wpsp-dropdown wpsp-dropdown-md wpsp-dropdown-right">
								<ul>
									<form id="ExportColumnForm" name="ExportParentColumn" method="POST">
										<li class="wpsp-drop-title wpsp-checkList"> <?php echo esc_html("Select Columns to Export","wpschoolpress");?></li>
										<?php foreach( $parentFieldList as $key=>$value ) { ?>
										<li class="wpsp-checkList" >
											<input type="checkbox" name="ParentColumn[]" value="<?php echo esc_attr($key); ?>" id="<?php echo esc_attr($key); ?>" checked="checked">
				...
				<?php } ?>
					...
					<div class="wpsp-btn-list"  <?php if($proversion['status'] != "1") {?> wpsp-tooltip="<?php echo esc_attr($protitle);?>" <?php } ?>>
						<div class="psp-dropdownmain wpsp-button-group">
							<button type="button" class="wpsp-btn print" id="ExportParents" <?php echo $prodisable;?> title="<?php echo esc_attr($protitle);?>"><i class="fa fa-download"></i> <?php echo esc_html("Export","wpschoolpress");?> </button>
							<button type="button" class="wpsp-btn wpsp-btn-blue wpsp-dropdown-toggle" <?php echo esc_html($prodisable);?> title="<?php echo esc_attr($protitle);?>">
								<!-- <span class="caret"></span>
								<span class="sr-only">Toggle Dropdown</span> -->
							</button>
