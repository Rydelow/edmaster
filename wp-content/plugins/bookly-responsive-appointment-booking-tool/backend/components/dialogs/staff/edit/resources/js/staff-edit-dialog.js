jQuery(function ($) {
    ...
    $('#bookly-js-new-staff')
        .on('click', function () {
                $('.bookly-js-save').addClass('newstaff');
            ...
            function (event, result) {
                ...
                if (waitResposes <= 0) {
                    ...
                    booklyAlert({success: [BooklyStaffEditDialogL10n.settingsSaved]})
                     ...
                     setTimeout(function () {
                        if(jQuery('.bookly-js-save').hasClass('newstaff')){
                              // let data = $staffList.DataTable().row($('[data-action="edit"]').closest('td')).data();
                              //   console.log(data);
                              //   staff_id = data.id;
                                

                                var inputs = jQuery(".cust-edit-popup");
                               
                                var current_staff_id = 0;
                                for(var i = 0; i < inputs.length; i++) {
                                    var value = jQuery(inputs[i]).val();
                                    var id = jQuery(inputs[i]).attr('data-id');
                                    id = parseInt(id);
                                ...
                                $('.bookly-js-save').removeClass('newstaff');
                                // console.log(current_staff_id);
